...
>
  <div
    class="bg-[white] rounded-full opacity-90 transition-opacity duration-700 absolute pointer-events-none hidden sm:block"
    class:w-10={size == 'sm'}
    class:h-10={size == 'sm'}
    class:w-20={size == 'md'}
    class:h-20={size == 'md'}
    ...
    style:transform="translate3d(calc({$coords.x}px - 50%), calc({$coords.y}px - 50%), 0)"
  />
  <div class="relative">
    <slot />
  </div>
